...
/**
 * Garis perjalanan: rel melengkung yang tumbuh saat digulir, dengan titik yang menyusurinya.
 *
 * Dipindah utuh dari `Story.vue` di fase 79 — nol perubahan perilaku. Ia varian termahal di
 * berkas mana pun di folder ini (ResizeObserver + DrawSVG + MotionPath), dan itulah alasan
 * kelimanya dimuat malas: undangan yang memilih wajah lain berhenti membayarnya.
 */
const props = defineProps<{ section: Section; steps: StoryStep[] }>()
const { galleryImages, initials, motionOptions } = useInvitation()

const root = ref<HTMLElement | null>(null)

const photoOf = (step: { image: string }, at: number) =>
  step.image || galleryImages.value[at % Math.max(1, galleryImages.value.length)] || ''

const stepCount = computed(() => Math.max(1, props.steps.length))
...
/*
 * `motionOptions` dioper sejak fase 79. Tanpa itu ScrollTrigger di sini membaca JENDELA sebagai
 * penggulung, padahal di panggung editor yang menggulung adalah layar ponsel — dan tombol
 * Statis tidak mematikan gerakan varian ini sama sekali.
 */
useArunaMotion(root, ({ gsap, drawSvg, travelPath }) => {
   ...
   */
  gsap.utils.toArray<HTMLElement>('[data-story-step]').forEach(node => {
...
<template>
  <div ref="root" class="grid w-full justify-items-center gap-6">
