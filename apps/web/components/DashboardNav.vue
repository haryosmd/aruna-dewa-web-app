...
const links = computed(() => [
  { label: 'Ringkasan', short: 'Ringkasan', to: `/dashboard/${props.invitationId}`, icon: LayoutDashboard },
  { label: 'Edit undangan', short: 'Edit', to: `/dashboard/${props.invitationId}/editor`, icon: PencilLine },
  { label: 'Kelola tamu', short: 'Tamu', to: `/dashboard/${props.invitationId}/guests`, icon: Users },
  { label: 'RSVP & ucapan', short: 'RSVP', to: `/dashboard/${props.invitationId}/rsvps`, icon: MessageSquareText },
  { label: 'Pesanan', short: 'Pesanan', to: `/dashboard/${props.invitationId}/orders`, icon: ReceiptText },
])
</script>
...
<template>
  ...
  <aside class="sticky top-0 hidden h-svh w-64 shrink-0 flex-col gap-6 border-r border-border bg-surface-2 p-5 lg:flex">
    <NuxtLink to="/dashboard" class="no-underline" aria-label="Aruna Dewa, ke daftar undangan">
      <BrandLogo />
    </NuxtLink>
