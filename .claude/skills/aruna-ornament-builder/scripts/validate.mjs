...
for(const a of catalog.assets){
  ...
  for(const v of a.variants){
    ...
    if(v.format==='svg'){
      ...
      // Sapuan/wash memang tidak pernah pekat: ia lapisan semi-transparan, bukan potongan.
      // Ambang "ada piksel pekat" karena itu hanya berlaku untuk cutout; untuk wash yang
      // diuji tetap ada daerah tembus DAN ada yang tergambar. Ditambahkan 2026-09-18 setelah
      // dua latar dari template Canva — sapuan kuas dan bidang lengkung — gagal dengan alpha
      // maksimum 107 dan 128, padahal keduanya memang begitu di sumbernya.
      const wash=/wash/i.test(a.style?.medium||'');
      const floor=wash?1:200;
      if(!meta.hasAlpha || stats.channels[3].min!==0 || stats.channels[3].max<floor)errors.push(`${a.id}: missing usable transparency`);
    }
  }
}
